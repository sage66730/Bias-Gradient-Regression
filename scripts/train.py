...
#export PYTHONPATH="/home/jefflo/Bias-Gradient-Regression"
...
def setup_dir(args, model_path):
    ...
    with open(f"{model_path}/info.txt", 'w') as fp:
        ...
        fp.write(f"bias_layer {args.bias_layer}\n")
        #fp.write(f" {args.}\n")
...
def print_info(args):
    ...
    print(f"\tbias_layer: {args.bias_layer}")
    print("**************************************************************")
    return
...
def wandb_setup(args, name):
    if args.wandb:
        ...
        wandb.init(project="BGR", entity="bogp", group=args.model, name=name, id=name)
        ...
        wandb.run.summary["bias_layer"] = str(args.bias_layer)
    return 
...
def gradient_on(freeze_epoch, model, e):
    ...
    if freeze_epoch >= 0 and e >= freeze_epoch:
                for name, param in model.named_parameters():
                    if name != "bias": param.requires_grad = True

def gradient_off(freeze_epoch, model, e):
    # freeze layers in network
    if freeze_epoch >= 0 and e >= freeze_epoch:
                for name, param in model.named_parameters():
                    if name != "bias": param.requires_grad = False

def train(args, model_path, device):
    # initialize
    model = get_model(args.model, args.bias_layer).to(device)
    ...
    for e in tq:
        ...
        for data, label in train_dataloader:
            ...
            pred = model(data)
            loss = loss_fn(pred, label, train_stat={"cur_epoch":e, "freeze_epoch":args.freeze_epoch,"alpha":args.alpha})
        ...
        # switch to SGD optimizer when training bias only
        if args.freeze_epoch >= 0 and e == args.freeze_epoch-1:
            optimizer = torch.optim.SGD(model.parameters(), lr=0.0001)
        
        # dynamic change learning rate for a bias regression
        if args.freeze_epoch >= 0 and e >= args.freeze_epoch-1:
            for param_group in optimizer.param_groups: 
                param_group['lr'] = max(loss.item(), 1.0e-7)
...
def metric(args, model_path):
    ...
    # load and calculate
    with open(f"{model_path}/loss_MD17SingleDataset_{args.style}_{args.molecule}{args.split}test.csv", newline='') as fp:
...
def main(args):
    ...
    run_name = f"BGR-{args.model}({start_dt})"
    model_path = f"../models/{args.model}/checkpoints/{run_name}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bias_layer", default=False, help="default: not to append a bias layer to the model")
    parser.add_argument("-B", "--bias", action="store_true", dest="bias_layer", help="append a bias layer to the model")
